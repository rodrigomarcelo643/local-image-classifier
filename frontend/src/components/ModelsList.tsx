...
import { modelToasts, genericToasts, showPromise, showLoading, closeToast } from "../utils/toast";
...
export default function ModelsList() {
  ...
  const [trainingStatus, setTrainingStatus] = useState<{is_training: boolean, progress: string}>({is_training: false, progress: ""});
  const [showImagesModal, setShowImagesModal] = useState(false);
  const [modalImages, setModalImages] = useState<{filename: string, filepath: string}[]>([]);
  ...
  const fetchModels = async () => {
    try {
      ...
      if (modelsRes.data.models?.length > 0 || trainingRes.data.training_data?.length > 0) {
        genericToasts.success(`Loaded ${modelsRes.data.models?.length || 0} models, ${trainingRes.data.training_data?.length || 0} trained labels, and ${uploadedRes.data.uploaded_data?.length || 0} uploaded labels`);
      }
    } catch (err) {
      console.error(err);
  ...
  const openTrainingModal = () => {
    if (uploadedData.length === 0) {
      genericToasts.error('No uploaded data available. Please upload some images first.');
      return;
    }
  ...
  const trainNewModel = async () => {
    ...
    try {
      await axios.post("http://localhost:8001/train", { labels: selectedLabels });
      ...
      const pollStatus = setInterval(async () => {
        try {
          const statusRes = await axios.get("http://localhost:8001/training-status");
      ...
      console.error(err);
      genericToasts.error("Model training failed. Please check your uploaded images.");
    }
  };
  ...
  const deleteModel = async (modelId: number, modelName: string) => {
    ...
    try {
      await showPromise(
        axios.delete(`http://localhost:8001/models/${modelId}`),
        {
  ...
  const downloadModel = async (model: Model) => {
    try {
      const loadingToast = showLoading(`Preparing ${model.name} for download...`);
  ...
  const viewImages = async (label: string, isTrainedData: boolean = false) => {
    try {
      const endpoint = isTrainedData ? `/training-images/${label}` : `/sample-images/${label}`;
    ...
    <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-green-100 text-lg">
              Manage and monitor your machine learning models ({models.length} model{models.length !== 1 ? 's' : ''} available)
            </p>
          </div>
          <button
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-green-100 p-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Search Models</label>
      ...
      {!models.length && !uploadedData.length && !trainingData.length ? (
        <div className="bg-white rounded-2xl shadow-lg p-12 text-center border border-green-100">
          ...
          <h3 className="text-2xl font-bold text-gray-800 mb-4">No Training Data Found</h3>
          <p className="text-gray-600 mb-6 max-w-md mx-auto">
            Start by uploading labeled images and training your first model to see data here.
          </p>
          <button 
            onClick={openTrainingModal}
                  ...
                  {uploadedData.map((data, index) => (
                    <div key={index} className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-semibold text-gray-800 text-lg">{data.label}</h4>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {data.sample_files.map((filename, imgIndex) => (
                            <div key={imgIndex} className="relative">
                              <img 
                                src={`http://localhost:8001/static/train/${data.label}/${filename}`}
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-green-100 overflow-hidden">
              <div className="p-6 border-b border-gray-200 flex justify-between items-center">
                <h3 className="text-xl font-semibold text-gray-800 flex items-center">
                  ...
                  Available Models {filteredModels.length > 0 && `(${filteredModels.length})`}
                ...
                <div className="px-6 py-4 bg-blue-50 border-b border-blue-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <svg className="animate-spin w-5 h-5 mr-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                      </svg>
                      <div>
                        <p className="font-medium text-blue-800">Training in Progress</p>
                        <p className="text-sm text-blue-600">{trainingStatus.progress}</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  {filteredModels.map((model) => (
                    <div
                      ...
                      className={`p-6 hover:bg-green-50 cursor-pointer transition-all duration-200 group ${
                        selectedModel?.id === model.id ? 'bg-green-50 border-l-4 border-green-500' : ''
                      }`}
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-3 mb-2">
                            <h4 className="text-lg font-semibold text-gray-800 truncate">{model.name}</h4>
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(model.status)}`}>
                              ...
                              <span className="ml-1 capitalize">{model.status || 'unknown'}</span>
                            </span>
                          </div>
                          
                          <p className="text-gray-600 text-sm mb-3 truncate">
                            <span className="font-medium">Path:</span> {model.path}
                          </p>
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                            {model.accuracy && (
                              <div className="flex items-center">
                                ...
                                <span className="text-xs font-medium text-gray-700 ml-2">{model.accuracy}%</span>
                              </div>
                            )}
                      ...
                      {(model.created_at || model.last_used) && (
                        <div className="flex flex-wrap gap-4 text-xs text-gray-500 mt-3">
                          {model.created_at && (
                            <div>
                              <span className="font-medium">Created:</span> {new Date(model.created_at).toLocaleDateString()}
                            </div>
                          )}
                          {model.last_used && (
                            <div>
                              <span className="font-medium">Last Used:</span> {new Date(model.last_used).toLocaleDateString()}
                            </div>
                          )}
                        </div>
                      )}
      ...
      {/* Model Details Modal/Sidebar */}
      {selectedModel && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-xl font-semibold text-gray-800">Model Details</h3>
              <button 
            ...
            <div className="p-6">
                  <div className="space-y-6">
                    {/* Basic Info */}
                    <div>
                      <label className="text-sm font-medium text-gray-500 block mb-2">Model Name</label>
                      <p className="text-lg font-semibold text-gray-800">{selectedModel.name}</p>
                    </div>
                    
                    <div>
                      <label className="text-sm font-medium text-gray-500 block mb-2">Storage Path</label>
                      <p className="text-sm text-gray-600 font-mono bg-gray-50 p-2 rounded break-all">{selectedModel.path}</p>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-sm font-medium text-gray-500 block mb-2">Model ID</label>
                      ...
                      <div>
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(selectedModel.status)}`}>
                          {getStatusIcon(selectedModel.status)}
                          <span className="ml-1 capitalize">{selectedModel.status || 'unknown'}</span>
                        </span>
                      </div>
                    </div>
                    ...
                    {selectedModel.accuracy && (
                      <div>
                        <label className="text-sm font-medium text-gray-500 block mb-2">Accuracy Score</label>
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-sm font-semibold text-gray-700 min-w-12">{selectedModel.accuracy}%</span>
                        </div>
                      </div>
                    )}

                    {/* Additional Info */}
                    {(selectedModel.classes || selectedModel.size) && (
                      <div className="grid grid-cols-2 gap-4">
                        {selectedModel.classes && (
                          <div>
                            <label className="text-sm font-medium text-gray-500 block mb-2">Classes</label>
                            <p className="text-sm text-gray-600">{selectedModel.classes.length}</p>
                          </div>
                        )}
                        {selectedModel.size && (
                          <div>
                            <label className="text-sm font-medium text-gray-500 block mb-2">Size</label>
                            <p className="text-sm text-gray-600">{selectedModel.size}</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {/* Timestamps */}
                    {(selectedModel.created_at || selectedModel.last_used) && (
                        ...
                        <div className="space-y-2 text-sm">
                          {selectedModel.created_at && (
                            <div className="flex justify-between">
                              ...
                              <span className="text-gray-800">{new Date(selectedModel.created_at).toLocaleDateString()}</span>
                            </div>
                          )}
                          {selectedModel.last_used && (
                            <div className="flex justify-between">
                              <span className="text-gray-600">Last Used:</span>
                              <span className="text-gray-800">{new Date(selectedModel.last_used).toLocaleDateString()}</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="pt-4 space-y-3">
                      <button 
                        onClick={() => useModel(selectedModel)}
                        className="w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition-colors duration-200 flex items-center justify-center space-x-2"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button 
                          ...
                          className="bg-white text-green-600 border border-green-600 py-2 rounded-lg font-medium hover:bg-green-50 transition-colors duration-200 flex items-center justify-center space-x-2"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                          </svg>
                        ...
                        <button 
                          ...
                          className="bg-white text-red-600 border border-red-600 py-2 rounded-lg font-medium hover:bg-red-50 transition-colors duration-200 flex items-center justify-center space-x-2"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                          </svg>
      ...
      {showTrainingModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-xl font-semibold text-gray-800">Select Training Data</h3>
              <button 
            ...
            <div className="p-6">
              <p className="text-gray-600 mb-6">Choose which labels to include in your model training:</p>
                ...
                {uploadedData.map((data, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-sm text-gray-600">{data.count} images</span>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                <div className="text-sm text-gray-600">
                  {selectedLabels.length} of {uploadedData.length} labels selected
                </div>
                ...
                <div className="flex space-x-3">
                  ...
                  <button
                    onClick={() => setSelectedLabels(uploadedData.map(data => data.label))}
                  ...
                  <button
                    ...
                    disabled={selectedLabels.length === 0 || trainingStatus.is_training}
                  ...
                  >
                    {trainingStatus.is_training ? 'Training...' : `Train Model (${selectedLabels.length} labels)`}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            {modalImages.length === 1 ? (
              ...
              <div className="p-6 flex justify-center">
                <img 
                  src={`http://localhost:8001/static/train/${modalLabel}/${modalImages[0].filename}`}
              ...
              <div className="relative">
                <div className="p-6 flex justify-center items-center min-h-96">
                  <img 
                    src={`http://localhost:8001/static/train/${modalLabel}/${modalImages[currentImageIndex].filename}`}
                    ...
                    {modalImages.map((image, index) => (
                      <button
                        ...
                        className={`flex-shrink-0 w-16 h-16 rounded border-2 overflow-hidden transition-all ${
                          index === currentImageIndex ? 'border-green-500' : 'border-gray-300 hover:border-gray-400'
                        }`}
                      >
                        <img 
                          src={`http://localhost:8001/static/train/${modalLabel}/${image.filename}`}
